...
# Re-export for callers that validate provider names
__all__ = [
...
ALLOWED_LLM_PROVIDERS: frozenset[str] = frozenset({"groq", "openai", "gemini"})


class Settings:
    """Application settings loaded from environment variables."""

    # LLM provider (groq | openai | gemini) — switched via env only
    LLM_PROVIDER: str = (os.getenv("LLM_PROVIDER", "groq") or "groq").strip().lower()
    # Prefer LLM_MODEL; fall back to GROQ_MODEL for existing .env files
    LLM_MODEL: str = (
        os.getenv("LLM_MODEL")
    ...
    # Kept for backward compatibility with older .env / docs
    ...
    GROQ_INGEST_MAX_TOKENS: int = _env_int("GROQ_INGEST_MAX_TOKENS", "1500")
    GROQ_INGEST_TEMPERATURE: float = _env_float("GROQ_INGEST_TEMPERATURE", "0.2")
    ...
    @property
    def active_llm_api_key_env(self) -> str:
        """Env var name for the active provider API key (for error messages)."""
        if self.LLM_PROVIDER == "openai":
    ...
    def validate(cls) -> None:
        ...
        if provider not in ALLOWED_LLM_PROVIDERS:
            ...
            missing.append(
                f"LLM_PROVIDER must be one of: {allowed} (got '{provider}')"
            )
        elif provider == "openai" and not (cls.OPENAI_API_KEY or "").strip():
            missing.append("OPENAI_API_KEY (required when LLM_PROVIDER=openai)")
        elif provider == "gemini" and not (cls.GEMINI_API_KEY or "").strip():
            missing.append("GEMINI_API_KEY (required when LLM_PROVIDER=gemini)")
        elif provider == "groq" and not (cls.GROQ_API_KEY or "").strip():
